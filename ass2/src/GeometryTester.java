...
public class GeometryTester {
    ...
    public boolean testPoint() {
        ...
        if (!doubleEquals(p1.getX(), 12)) {
            System.out.println("Test.java p1.getX() failed.");
            mistake = true;
        }
        if (!doubleEquals(p1.getY(), 2)) {
            System.out.println("Test.java p1.getY() failed.");
            mistake = true;
        }
        if (!doubleEquals(p1.distance(p1), 0)) {
            System.out.println("Test.java distance to self failed.");
            mistake = true;
        }
        if (!doubleEquals(p1.distance(p2), p2.distance(p1))) {
            System.out.println("Test.java distance symmetry failed.");
            mistake = true;
        }
        if (!doubleEquals(p1.distance(p2), 5)) {
            System.out.println("Test.java distance failed.");
            mistake = true;
        }
        if (!p1.equals(p1)) {
    ...
    public boolean testLine() {
        ...
        if (!l1.isIntersecting(l2)) {
            System.out.println("Test.java isIntersecting failed (1).");
            mistakes = true;
        }
        if (l1.isIntersecting(new Line(0, 0, 1, 1))) {
            System.out.println("Test.java isIntersecting failed (2).");
            mistakes = true;
        }
        Point intersectL1L2 = l1.intersectionWith(l2);
        if (!l1.middle().equals(intersectL1L2)) {
            System.out.println("Test.java intersectionWith middle failed.");
            mistakes = true;
        }
    ...
    public static void main(String[] args) {
        ...
        if (tester.testPoint() && tester.testLine()) {
            System.out.println("Test.java Completed Successfully!");
        }
        else {
